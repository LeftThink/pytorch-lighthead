...
def adas_eval(detpath,
  ...
  with open('pr.800.txt','w') as out:
    for i in range(nd):
      tp_l1_mean = l1_c[i]/tp_c[i]
      '''
      out.write("gt: %d image_id: %-30s tp: %d fp: %d iou: %.3f tp_c: %-4d fp_c: %-4d score: %.3f pre: %.3f rec: %.3f, l1_x0: %.3f, l1_y0: %.3f, l1_x1: %.3f, l1_y2:%.3f\n" \
          % (npos, image_ids[i], tp[i], fp[i], iou[i], tp_c[i], fp_c[i], -sorted_scores[i], prec[i], rec[i], tp_l1_mean[0], tp_l1_mean[1], tp_l1_mean[2], tp_l1_mean[3]))
      '''
